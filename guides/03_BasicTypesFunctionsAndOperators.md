...
This is the very unique datatype that exists in Erlang, and as a note for Haskell users; `Binary` contains the same information as `ByteString`. If you are not very familiar with binaries, this [link](https://erlang.org/doc/man/binary.html) could be helpful for some clearification.
